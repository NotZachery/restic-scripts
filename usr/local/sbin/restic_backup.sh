...
BACKUP_PATHS="/home/minecraft /etc/nginx /etc/iptables /var/www /etc/postfix /etc/dovecot /etc/rspamd /usr/local/sbin /etc/systemd/system"
...
mysqldump -u restic -pletmein --all-databases | restic backup \
	--verbose \
	--tag mysqldbs \
	--stdin \
	--stdin-filename mysql_databases.sql &
wait $!
...
# Send a message to a discord channel stating the backup is complete
curl -H "Content-Type: application/json" -d "{\"content\": \"[$(date)] Restic backup complete\"}" https://discordapp.com/api/webhooks/<CHANNELID>/<DISCORD>
